...
<td>必须是已经发布的小程序存在的页面（否则报错），例如 "pages/index/index" ,根路径前不要填加'/',不能携带参数（参数请放在scene字段里），如果不填写这个字段，默认跳主页面</td>
...
**注：pages/index 需要在 `app.json` 的 [`pages`](../framework/config.html#pages) 中定义**
...
[](share.html#页面内发起转发)[](address.html)</div>
