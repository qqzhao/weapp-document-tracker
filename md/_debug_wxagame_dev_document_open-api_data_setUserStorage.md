...
上报用户数据后台接口。小游戏可以通过本接口上报key-value数据到用户的CloudStorage。注意用户的存储数据量不超过**1M**，单个key-value对数据不能超过**1K**。
...
<td>用户登录态签名，签名算法请参考[用户登录态签名算法](../../../tutorial/open-ability/http-signature.html)</td>
...
<td>用户登录态签名的哈希方法，如hmac_sha256等，请参考[用户登录态签名算法](../../../tutorial/open-ability/http-signature.html)</td>
...
[](removeUserStorage.html)[](../context/wx.getOpenDataContext.html)</div>
